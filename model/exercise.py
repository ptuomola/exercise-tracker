...
def insert_exercise(
    activity_id, start_date, start_time, end_date, end_time, description, external_url
):
    retval = db.session.execute(
        """INSERT INTO exercises (activity_id, user_id, start_date, start_time, end_date,
                                  end_time, description, external_url)
                          VALUES (:activity_id, :user_id, :start_date, :start_time, :end_date,
                                  :end_time, :description, :external_url )
                          RETURNING id""",
        {
...
def get_exercises_for_user(user_id):
    return db.session.execute(
        """SELECT e.*, a.description AS activity_description, a.activity_type AS activity_type,
                                    (SELECT COUNT(1) FROM subactivities sa
                                     WHERE sa.activity_id = e.activity_id)
                                     as num_subactivity_types
...
def get_exercise_by_id(exercise_id):
    return db.session.execute(
        "SELECT * FROM exercises WHERE id = :exercise_id", {"exercise_id": exercise_id}
    ).first()


def delete_exercise_by_id(exercise_id):
    db.session.execute(
        "DELETE FROM exercises WHERE id = :exercise_id", {"exercise_id": exercise_id}
    )
    db.session.commit()


def update_exercise(
    exercise_id, start_date, start_time, end_date, end_time, description, external_url
):
    db.session.execute(
        """UPDATE exercises
...
def get_subactivities_for_exercise(exercise_id):
    return db.session.execute(
        """SELECT esa.*, s.description as description
                          FROM exercise_subactivities esa, subactivities s
                          WHERE exercise_id = :exercise_id
                          AND esa.subactivity_id = s.id""",
        {"exercise_id": exercise_id},
    )
...
def insert_exercise_subactivity(exercise_id, subactivity_id, amount):
    retval = db.session.execute(
        """INSERT INTO exercise_subactivities (exercise_id, subactivity_id, count)
...
def delete_exercise_subactivity_by_id(subactivity_id):
    db.session.execute(
        "DELETE FROM EXERCISE_SUBACTIVITIES WHERE id = :subactivity_id",
...
def get_subactivity_exercise_by_id(subactivity_id):
    return db.session.execute(
        "SELECT * FROM exercise_subactivities WHERE id = :subactivity_id",
        {"subactivity_id": subactivity_id},
    ).first()
